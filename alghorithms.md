...
$$
\mathbf{f}_i = (f_{i1}, f_{i2}), \qquad (x_i, y_i).
$$
...
\mathbf{w}_x^{*}
= \arg\min_{\mathbf{w}}
\Bigl( \bigl\| X \mathbf{w} - \mathbf{x} \bigr\|_2^2
      + \lambda \bigl\| \mathbf{w} \bigr\|_2^2 \Bigr),
$$

$$
\mathbf{w}_y^{*}
= \arg\min_{\mathbf{w}}
\Bigl( \bigl\| X \mathbf{w} - \mathbf{y} \bigr\|_2^2
      + \lambda \bigl\| \mathbf{w} \bigr\|_2^2 \Bigr).
$$
...
To control jitter from raw gaze estimates, the calibrated coordinates are passed through an exponential moving average (one-pole IIR filter). The smoothed position \( \mathbf{g}_t \) is defined as

$$
\mathbf{g}_t
= (1 - \alpha)\, \mathbf{g}_{t-1}
+ \alpha\, \mathbf{g}^{\text{raw}}_t .
$$

Here \( \mathbf{g}^{\text{raw}}_t \) is the instantaneous calibrated position at time \( t \); \( \mathbf{g}_t \) is the smoothed position; and \( \alpha \) in \( (0, 1) \) controls the trade-off between jitter suppression and latency (typical values are around \( 0.3 \) to \( 0.5 \)).

For outlier rejection we compare successive positions and ignore large jumps. If the step size

$$
\bigl\| \mathbf{g}^{\text{raw}}_t - \mathbf{g}_{t-1} \bigr\|
$$

exceeds a defined jump threshold (saccade detection), the sample is clamped or ignored for the dwell detector. This prevents short-lived tracking glitches or fast saccades from triggering spurious note events.

---

### 3. Dwell-Based Note Selection

For a given smoothed position we compute the active grid cell index \( c_t \).
...
where \( d_{\min} \) is a selection threshold (for example, \( 100 \text{ ms} \)). Sub-threshold dwells are treated as noise or saccades and ignored.
...
Given a dwell duration \( d \), we first normalize to the range \([0, 1]\):
...
where \( d_{\min} \) and \( d_{\max} \) are lower and upper reference points for “short” and “long” dwells in performance.
