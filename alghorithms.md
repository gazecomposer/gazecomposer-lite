...
$$
\phi(\mathbf{f}_{i}) = \left[\, 1,\ f_{i1},\ f_{i2},\ f_{i1}^2,\ f_{i1} f_{i2},\ f_{i2}^2 \,\right]
$$
...
$$
\mathbf{w}_{x}^* = \arg\min_{\mathbf{w}} \left( \| X \mathbf{w} - \mathbf{x} \|_{2}^2 + \lambda \| \mathbf{w} \|_{2}^2 \right)
$$

$$
\mathbf{w}_{y}^* = \arg\min_{\mathbf{w}} \left( \| X \mathbf{w} - \mathbf{y} \|_{2}^2 + \lambda \| \mathbf{w} \|_{2}^2 \right)
$$
...
$$
u = \text{clamp}\left( \frac{d - d_{\min}}{d_{\max} - d_{\min}},\ 0,\ 1 \right)
$$
...
* **Conservative vibrato (late onset, shallow):**
  ...
  \begin{cases} 
     0 & \text{if } u < u_{0} \\
     \left( \frac{u - u_{0}}{1 - u_{0}} \right)^{\gamma_{\text{cons}}} & \text{if } u \ge u_{0} 
  \end{cases}
  $$
